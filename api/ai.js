...
const KEY=process.env.AI_KEY||(K.AI_E?decKey(K.AI_E):(K.AI_B64?Buffer.from(K.AI_B64,'base64').toString('utf8'):''));
...
if(req.method!=='POST'||!b.user)return res.status(400).json({error:'empty_user'});
var p=KEY.indexOf('gsk_')===0?{url:'https://api.groq.com/openai/v1/chat/completions',pref:['llama-3.3-70b-versatile','meta-llama/llama-4-scout-17b-16e-instant','openai/gpt-oss-120b','openai/gpt-oss-20b','llama-3.1-8b-instant']}:{url:'https://api.x.ai/v1/chat/completions',pref:['grok-3-mini','grok-3','grok-4','grok-2-latest']};
