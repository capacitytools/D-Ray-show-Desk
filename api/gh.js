...
const TOK=process.env.GH_TOKEN||(K.GH_B64?Buffer.from(K.GH_B64,'base64').toString('utf8'):'');
...
try{
 ...
 var sha;
 var g2=await fetch('https://api.github.com/repos/'+REPO+'/contents/'+b.path,{headers:H}); if(g2.ok){var m2=await g2.json();sha=m2.sha}
